DROP PROCEDURE IF EXISTS createRegIndexIfNotExists#
CREATE PROCEDURE createRegIndexIfNotExists()
BEGIN
	declare indexExists int default 0;
	SELECT COUNT(*) into @indexExists
		FROM INFORMATION_SCHEMA.STATISTICS
		WHERE table_name = 'rpt_recent_registrations'
		and	index_name = 'IDX_ACCOUNT_ID';
		
	if @indexExists <> 1 then
		ALTER TABLE rpt_recent_registrations 
		ADD INDEX IDX_ACCOUNT_ID (ACCOUNT_ID ASC) ;
	end if;
END#
CALL createRegIndexIfNotExists()#
DROP PROCEDURE createRegIndexIfNotExists#
