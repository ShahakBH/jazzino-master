...
begin
  ...
  -- delete targets from requests that have been sent and expired at least 1 day ago or 7 days where expiry date is null (to allow for delete requests to be sent)
  declare appRequestIdCursor cursor for select id from APP_REQUEST where sent is not null and ((expiry_dt is null and sent < date_sub(now(), interval 7 day)) or (expiry_dt < date_sub(now(), interval 1 day)));
