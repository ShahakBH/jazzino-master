...
begin
    ...
    insert into strataproddw.APP_REQUEST (channel_type, title, description, message, tracking, created, scheduled_dt, expiry_dt)
    select
        channel_type,
        game_type title,
    ...
    from tmp_app_request_template t, (select 'FACEBOOK_APP_TO_USER_REQUEST' channel_type union select 'IOS' channel_type union select 'GOOGLE_CLOUD_MESSAGING_FOR_ANDROID' channel_type) tc
    ...
    and ar.channel_type = 'FACEBOOK_APP_TO_USER_REQUEST'
    ...
    select distinct
        ...
        null,
        mt.game_type
    from tmp_app_request_target mt, strataproddw.GCM_PLAYER_DEVICE d, strataproddw.APP_REQUEST ar
    where mt.player_id = d.player_id
    and mt.game_type = d.game_type
    and ar.description = concat('Progressive Bonus Reminders ', mt.game_type, ' ', curdate())
    and ar.channel_type = 'GOOGLE_CLOUD_MESSAGING_FOR_ANDROID';
