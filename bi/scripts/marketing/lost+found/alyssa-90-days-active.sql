...
drop table if exists paranoid;

create table paranoid as
select distinct
    mt.game_type,
    mt.player_id
from tmp_app_request_target mt, strataproddw.GCM_PLAYER_DEVICE d
where mt.player_id = d.player_id
and mt.game_type = d.game_type;
...
mysql -uroot -e "select * from damjan.paranoid" | sed "s/\s/,/g" > android_90.csv
zip /tmp/app.zip fb_90.csv ios_90.csv android_90.csv
