...
when "redhat", "centos", "scientific", "oracle"
  ...
  if node['postgresql']['version'].to_f == 9.3
    ...
    default['postgresql']['client']['packages'] = %w{postgresql93 postgresql93-devel postgresql93-libs}
...
when 'rhel', 'fedora', 'suse'
  ...
  default['postgresql']['config']['default_text_search_config'] = 'pg_catalog.english'
  default['postgresql']['config']['standard_conforming_strings'] = 'false'
end

# Modification by dseeto

default['postgresql']['pg_hba'] = [
  {:type => 'local', :db => 'all', :user => 'all', :addr => nil, :method => 'trust'},
  {:type => 'host', :db => 'all', :user => 'spanner', :addr => '127.0.0.1/32', :method => 'ident map=reporting'},
  {:type => 'host', :db => 'all', :user => 'spanner', :addr => '::1/128', :method => 'ident map=reporting'},
  {:type => 'host', :db => 'all', :user => 'all', :addr => '127.0.0.1/32', :method => 'md5'},
  {:type => 'host', :db => 'all', :user => 'all', :addr => '::1/128', :method => 'md5'}
]

default['postgresql']['allowed_networks'] = ['151.237.239.34/32', '10.176.0.0/16', '10.9.8.0/24', '192.168.0.0/24', '192.168.1.0/24', '5.39.67.78/32', '5.39.65.6/32', '5.39.67.33/32', '109.74.203.53/32', '212.71.255.65/32']
node['postgresql']['allowed_networks'].each do |network|
  default['postgresql']['pg_hba'] << {:type => 'host', :db => 'all', :user => 'all', :addr => network, :method => 'md5'}
end

default['postgresql']['pg_ident'] = [
  {:mapname => 'reporting', :system_username => 'spanner', :pg_username => 'postgres'},
  {:mapname => 'reporting', :system_username => 'spanner', :pg_username => 'admin'},
  {:mapname => 'reporting', :system_username => 'spanner', :pg_username => 'reporting'}
]
